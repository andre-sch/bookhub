import { iso_639_1 } from "./1760899267834_add-languages";
...
export const up = transaction(async (client: Client) => {
  ...
  await insert(iso_639_3, client);
});

export const down = transaction(async (client: Client) => {
  await client.query("DELETE FROM language;");
  await insert(iso_639_1, client);
});

async function insert(language_set: [code: string, language: string][], client: Client) {
  const insertions = language_set.map((_, index) => `($${2*index + 1}, $${2*index + 2})`);
  await client.query(
...
export const iso_639_3: [
  ...
  ["hbn", "Heiban"],
  ["hbo", "Ancient Hebrew"],
  ["hbs", "Serbo-Croatian	Code element for 639-1 has been deprecated"],
  ["hbu", "Habu"],
